...
// Check streamers informations every 2.5 seconds and update
setInterval(function () {
  console.log("Check streamers...");
  // Change extension icon depending on favorite streamer status
  chrome.storage.local.get("streamers", function (data) {
    if (data.streamers.length >= 1 && data.streamers[0].status == true) {
      chrome.browserAction.setIcon({ path: "icons/online-icon-16.png" });
    } else {
      chrome.browserAction.setIcon({ path: "icons/offline-icon-16.png" });
    }
...
async function postData(url = "", type, pass = "") {
  ...
  xhr.onreadystatechange = function () {
    ResponseHandler(this, type, pass);
  };
  xhr.send();
}

// Function to handle get request response
function ResponseHandler(resp, type, pass) {
  if (!type) {
    if (resp.readyState !== 4) return;
    if (resp.status !== 200) return; // error handling
    const ParseText = JSON.parse(resp.responseText);
    UpdateStreamerLogo(
      ParseText.users[0].name,
      ParseText.users[0].logo,
      ParseText.users[0]._id
    );
    postData(
      "https://api.twitch.tv/kraken/streams/" + ParseText.users[0]._id,
      1,
      ParseText.users[0].name
    );
  } else {
    ...
    const ParseText = JSON.parse(resp.responseText);
    if (ParseText.stream) {
      UpdateStreamer(pass, true);
      return;
    }
    UpdateStreamer(pass, false);
  }
}
...
// Function to update streamer logo
